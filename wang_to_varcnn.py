...
def process_trace(args):
    ...
    dir_seq = torch.zeros(5000, dtype=torch.int8)
    time_seq = torch.zeros(5000, dtype=torch.float32)
    ...
    for packet_num, line in enumerate(lines):
        ...
        curr_dir = torch.sign(torch.tensor(int(line[1]))).item()
    ...
    if total_packets == 0:
        metadata = torch.zeros(7, dtype=torch.float32)
    else:
        metadata = torch.tensor([total_packets, total_incoming, total_outgoing,
...
def main(config):
    ...
    dir_seq_mon = torch.stack(dir_seq_mon).cpu().numpy()
    time_seq_mon = torch.stack(time_seq_mon).cpu().numpy()
    metadata_mon = torch.stack(metadata_mon).cpu().numpy()
    ...
    dir_seq_unmon = torch.stack(dir_seq_unmon).cpu().numpy()
    time_seq_unmon = torch.stack(time_seq_unmon).cpu().numpy()
    metadata_unmon = torch.stack(metadata_unmon).cpu().numpy()
